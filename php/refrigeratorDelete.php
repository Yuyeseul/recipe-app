...
// URL 경로에서 nickname과 ingredientName 가져오기
if (isset($_SERVER['PATH_INFO'])) {
    $path = explode('/', $_SERVER['PATH_INFO']);
    if (count($path) == 3) { // 3개 인자가 있는지 확인
        $nickname = $path[1];
        $ingredientName = $path[2];

        // SQL 쿼리 준비 및 실행
        $statement = mysqli_prepare($con, "DELETE FROM refrigerator WHERE nickname = ? AND ingredientName = ?");
        mysqli_stmt_bind_param($statement, "ss", $nickname, $ingredientName);
        mysqli_stmt_execute($statement);

        $response = array();
        $response["success"] = mysqli_stmt_affected_rows($statement) > 0;
        $response["message"] = $response["success"] ? "Refrigerator deleted successfully" : "No refrigerator found";
