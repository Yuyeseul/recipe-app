...
if (isset($_POST['nickname']) && isset($_POST['recipeName'])) {
    ...
    // SQL 쿼리 준비 및 실행
    $statement = mysqli_prepare($con, "DELETE FROM bookmark WHERE nickname = ? AND recipeName = ?");
    mysqli_stmt_bind_param($statement, "ss", $nickname, $recipeName);
    mysqli_stmt_execute($statement);

    $response = array();
    $response["success"] = mysqli_stmt_affected_rows($statement) > 0;
    $response["message"] = $response["success"] ? "Bookmark deleted successfully" : "No bookmark found";
